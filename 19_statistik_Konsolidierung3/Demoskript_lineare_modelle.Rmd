...
# Statistik Konsolidierung 3 (16.11.2020)
...
>Download [R-Skript](19_Statistik_Konsolidierung3/demoskript_lineare_modelle.R)

*****


```{r anova & ancova, message=FALSE, eval=FALSE}
# für mehr infos
...
aov.2 <- aov(log10(hp) ~ cyl * am + wt, data = d) 
par(mfrow = c(2,2))
plot(aov.2)
summary.lm(aov.2)
...
ggplot(d, aes(x = cyl, y = hp)) +
  ...
  labs(x = "Anzahl Zylinder", y = "Pferdestärke")  + mytheme
...
new.data <- tibble(mpg = sample(seq(15.1,33.9, .3), 31),
...
model_ad <- lm(hp ~ carb + disp + mpg + wt, data = d)
