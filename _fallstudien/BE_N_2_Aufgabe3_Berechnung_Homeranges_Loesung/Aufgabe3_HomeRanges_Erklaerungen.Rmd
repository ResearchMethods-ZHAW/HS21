---
title: "Homeranges Fallstudie WPZ Loesungen"
author: "B.Sigrist"
...
## Benötigte Libraries laden, hier mit Hilfe einer Funktion, die vor dem Laden die entsprechenden Packages installiert, falls dies noch nicht der Fall sein sollte.
...
packages <- c("sf", "raster", "tidyverse", "adehabitatHR", "maptools", "sp", "ggspatial", "rgeos", "rgdal")
ipak(packages)
...
Rehe <- read_delim("Data/Rehe_landforst_20201020.csv", delim = ";")

str(Rehe)

```

## Aufgabe 1: In Datensatz Rehe eine neue Spalte mit Datum und Zeit in einer Spalte kreieren. Beim Format hat sich ein Fehler eingeschlichen. 

### -> Fehler: Format = d%.%m.%Y %H:%M:%S 

```{r tidy=TRUE, eval=TRUE}

Rehe <- Rehe %>%
  mutate(UTC_DateTime = as.POSIXct(paste(UTC_Date, UTC_Time), format = "d%.%m.%Y %H:%M:%S"))
...
st_write(hr, dsn= "Results", layer="HR_RE13", driver="ESRI Shapefile",  delete_layer = T )
...
ggplot(hr, aes(color = "red", fill = "red")) +
  ...
  geom_sf(size = 1, alpha = 0.3, colour="red") +
  scale_fill_manual(values="red") +
  geom_sf(data = xy_p, colour="black") +
  coord_sf(datum = sf::st_crs(21781)) +
...
ggplot(grid_plot) +
      annotation_spatial(pk25_wpz, alpha = 0.75) +
  geom_sf(color = "black", fill=NA) +
geom_sf(data = xy_p, color = "blue", size = 2) +
  geom_sf(data = hr, color = "red", fill = NA, size = 1.2) +
coord_sf(datum = sf::st_crs(21781))+
theme(
axis.title = element_blank(),
axis.text = element_blank(),
...
## Aufgabe 3 (28.10: fakultativ): testen der Variablen der Vegetationsschichten von letzter Woche auf einen linearen Zusammenhang (Korrelation; Funktion cor.test). DG_Baumschicht vs. DG_os / DG_Strauchschicht vs. DG_us aus dem Datensatz df_with_lidar den wir letzte Woche erstellt haben

## Die Theorie zu Korrelation folgte erst 27.10. Vormittag

```{r tidy=TRUE,message=FALSE}

df_with_lidar <- read_delim("Results/df_with_lidar.csv", delim =";")
