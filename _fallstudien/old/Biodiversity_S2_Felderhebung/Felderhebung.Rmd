...
Es gibt eine Vielzahl an möglichen Methoden zur Erfassung der Besucherzahlen. Automatische Zählgeräte bieten die Möglichkeit lange und durchgehende Zeitreihen zu erfassen. Managemententscheide können mithilfe dieser fundierten Datengrundlagen gezielt geplant und gerechtfertigt werden.

## Ziele

In dieser Übung wird eine Methode des Besuchermonitorings angewandt. 
...
Die BEsuchenden werden über den Zweck der Kameras informiert:
...
Lese nun zuerst den bereitsgestellen Datensatz (csv) mithilfe von __fread()__ oder __read.csv()__ ein und nennt ihn __cam__.
...
ggplot(Begleit_ost,                      # hier den Datensatz spezifizieren
  ...
  labs(x="Begleittier", y= "Anzahl")+    # Achsenbeschriftung setzen
  theme_classic(base_size = 20)+         # Und zu guter letzt: Stil definieren
  theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0)) # sowie Achsenbeschr.ausrichten
```

Bei Bedraf koennen die selben Plots fuer den zweiten Standort gemacht werden.

Und schliesslich: Wildtier als Bar Chart
