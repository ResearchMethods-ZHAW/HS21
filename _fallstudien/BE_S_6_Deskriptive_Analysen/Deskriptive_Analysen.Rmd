...
Nachdem wir die Projektstruktur aufgebaut haben und die Daten importiert/vorbereitet sind, machen wir uns an die deskriptive Analyse (= offene, beschreibende Statistik). Dies macht eigentlich immer Sinn. Bevor mach sich an die schliessende Statistik macht, muss man ein "Gefühl" für die Daten bekommen. Dies funktioniert eben am einfachsten mit deskriptiven, explorativen Analysen. Was wir hier machen ist __Informations-Visualisierung.__

Wir interessieren uns in den Analysen zum Einfluss von Covid-19 auf die Besuchszahlen für 4 Zeitabschnitte:

1) Normal (vom Start der Erhebungen bis vor dem ersten Lockdown)
2) Lockdown 1
3) Lockdown 2
4) Seit Covid-19, aber nicht während eines Lockdowns
...
- Plottet den Verlauf der monatlichen Besuchszahlen an eurer Zählstelle. Auf der x-Achse steht dabei dabei Jahr und Monat (gespeichert im df _depo_m_), auf der y-Achse die monatlichen Besuchszahlen. Zeichnet auch die beiden Lockdowns ein.
...
ggplot(depo_m, mapping = aes(Ym, Total, group = 1))+ # group 1 braucht R, dass aus den Einzelpunkten ein Zusammenhang hergestellt wird
  ...
  scale_x_discrete(breaks = c("2019 1", "2019 7","2019 1","2020 1","2020 7","2021 1","2021 7"),
                   labels = c("2019 1", "2019 7","2019 1","2020 1","2020 7","2021 1","2021 7"))+
  labs(title= "", y="Fussgänger:innen pro Monat", x = "Jahr / Monat")+
  theme_linedraw(base_size = 15)+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
```
...
```{r eval = F}
ggplot(data = depo_m, mapping = aes(x=Ym, y=Total, group = 1))+ # für was braucht es group=1???
  
  # zeichne Lockdown 1; ein einfaches Rechteck. bestimme mit min und max die Dimensionen
  geom_rect(...)+ # das Rechteck soll 10 % grösser als die maximale Besuchszahl sein 
...
- Berechnet zuerst die den Durchschnitt der Besuchszahlen pro Wochentag pro Phase.
...
- Prüft das pro Phase mit einem einfachen statistischen Test.

__Hinweis:__ Die Website der UZH hilft sehr übersichtlich weiter, wenn man nicht weiss, welchen Test man anwenden kann:

https://www.methodenberatung.uzh.ch/de.html

Der dort gezeigte Entscheidbaum ist ein interaktiver Zugang zu Methoden der statistischen Datenanalyse.
...
- Wir müssen also noch transformieren:
...
__Hinweis:__ melt ersetzt gather. Gather und spread werden nicht mehr gewartet / betreut
...
```{r echo=F} 
ggplot(subset(Mean_h_long, Phase %in% c("Normal")), 
                     mapping=aes(x = Stunde, y = Durchschnitt, colour = Wochentag, linetype = Wochentag))+
  geom_line(size = 2)+
  scale_colour_viridis_d()+
  scale_linetype_manual(values = c(rep("solid", 5),  "twodash", "twodash"))+
...
- transformiert beide df mittels __reshape2::melt()__ von wide zu long (siehe auch Codeschnipsel unter 3a)
