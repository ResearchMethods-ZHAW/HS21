...
- Schliesslich schneiden wir den Datensatz auf die Untersuchungsdauer zu. Falls euer Datensatz von Anfang bis Ende verfügbare Messwerte hat, dann ist dieser letzte Schritt nicht nötig (da der erste Tag und der letzte Tag ja bereits = depo_start und depo_end). 

```{r eval=FALSE}
str(depo)

depo <- depo %>%
  mutate(DatumUhrzeit = as.character(DatumUhrzeit)) %>%
  separate(DatumUhrzeit, into = c("Datum", "Zeit"), sep = " ")%>% # mit seperate() trennt man
                                                                   # 1 Spalte in 2.
  ...
  filter(Datum >= depo_start, Datum <=  depo_end) # das Komma hat die gleiche Funktion wie ein &
                                                  # depo_start und depo_end wurden in der Projektierung definiert
```
...
- Lest die Meteodaten (order_97149_data.txt) ein und speichert sie unter __meteo__.
