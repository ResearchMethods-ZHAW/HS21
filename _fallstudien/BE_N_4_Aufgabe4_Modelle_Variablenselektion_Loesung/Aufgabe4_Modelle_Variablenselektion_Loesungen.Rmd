---
title: "Variablenselektion Fallstudie WPZ Loesungen"
author: "B.Sigrist"
...
packages <- c("sp", "raster", "tidyverse", "PerformanceAnalytics", "pastecs", 
              "lme4", "bbmle", "MuMIn", "magrittr")
...
## Aufgabe 1: Mit dem folgenden Code kann eine simple Korrelationsmatrix aufgebaut werden. forest (0/1) ignorieren wir, dies ist nur eine convenience variable, die angibt, ob der Kreis im Wald liegt oder nicht.

```{r tidy=TRUE, message=FALSE}

DF_mod <- read_delim("Data/Datensatz_Habitatnutzung_Modelle_20201102_moodle.csv", delim = ";")
...
# hier kann die Schwelle fuer die Korrelation gesetzt werden, 0.7 ist liberal / 0.5 konservativ
...
# und wie schaut es bei der Verteilung der Daten der Abhaengigen Variable Nutzungsintensitaet aus?
...
# Die erklaerende Variable in m1 ist die erste Variable der korrelierdenen Beziehung
# Die erklaerende Variable in m2 ist die zweite Variable der korrelierdenen Beziehung

m1 <- glmer(Abhaengige_Variable ~ Erklaerende_Variable + (1 | id), data = DF_mod_day, family = binomial)
m2 <- glmer(Abhaengige_Variable ~ Erklaerende_Variable + (1 | id), data = DF_mod_day, family = binomial)

# mit dieser Funktion koennen die Modellergebnisse inspiziert werden
summary(m1)

# Mit dieser Funktion kann der Informationgehalt der beiden Modelle gegeneinander abgeschaetzt werden
bbmle::AICtab(m1, m2)
...
# --> tiefer AIC -> besser == Distanz zu Strassen und Wegen

# ==> dieses Vorgehen muss nun für alle korrelierten Variablen für jeden Teildatensatz (Tag/Nacht) durchgeführt werden, um nur noch nicht (R < 0.7) korrelierte Variablen in das Modell einfliessen zu lassen 

```

## Selektion der Variablen in einem multivariaten Model

### Aufgabe 5: Mit folgendem Code kann eine automatisierte Variablenselektion (dredge-Funktion) und ein Modelaveraging aufgebaut werden (siehe auch Vorlesung von J.Dengler vom 4.11. Vormittag)

```{r tidy=TRUE, eval=T}

# hier wird die Formel für die dredge-Funktion vorbereitet (die Variablen sind jene welche nach der univariaten Variablenselektion noch übrig bleiben)  
...
# inn diesem Befehl kommt der Random-Factor (das Reh) hinzu und es wird eine Formel daraus gemacht
...
# Importance values der einzelnen Variablen (Gibt an, wie bedeutsam eine bestimmte Variable ist, wenn man viele verschiedene Modelle vergleicht (multimodel inference))
...
# ==> für den Nachtdatensatz muss der gleiche Prozess der Variablenselektion durchgespielt werden. 
```
