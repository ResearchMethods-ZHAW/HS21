---
title: "KW40 - KW42: Aufgabe Felderhebung Grüental"
description:
author:
...
Es gibt eine Vielzahl an möglichen Methoden zur Erfassung der Besuchszahlen. Automatische Zählgeräte bieten die Möglichkeit lange und durchgehende Zeitreihen zu erfassen. Diese müssen aber natürlich auch ausgewertet werden. Hier erhaltet ihr erste Inputs dazu.
...
Nun geht es ins Feld und die Geräte werden installiert.
...
Für diese Aufgabe benötigen wir folgende Bibliotheken:
...
Unser Datensatz enthällt die Angeben zu __ost__ und __west__. Wir wollen die Auswertungen aber pro Standort machen.
...
- Prüfe daher die Verteilung pro Datensatz mittels __Histogram__ und __Scatterplot__.

Beim Histogram sollen nur die Menschen angezeigt und die 0er ausgeschlossen werden. Wir wollen keine Wildtiere im Plot. 
Das kann mit folgendem Code erreicht werden:

```{r eval=FALSE}
hist(west$Anzahl[west$Art=="Mensch" &
                  !west$Anzahl==0], # das "!" bedeutet "nicht gleich"
     breaks = 100)                  # wie viele Balken brauchen wir im Histogram?
```
...
Für die weiteren Analysen schliessen wir die Wildtiere komplett aus.

- Nutzt dazu wiederum den Befehl __filter()__ und überschreibt die Datensaetze __ost__ und __west__.

Dennoch wolle wir auch wissen, welche Tiere auf dem Areal (ost und west zusammen, also df __cam__) unterwegs sind.

- Dafür gibts einen separaten Datensatz namens __Tiere__. Nutzt dazu den Befehl __filter()__.

### Aufgabe 4: Explorative Analysen

- Berechnet die totale Anzahl Menschen / Standort mit __sum(DATENSATZ$SPALTENNAME)__.

- Berechnet die Anzahl Menschen pro Aktivität sowie die Anzahl Begleittier pro Kategorie pro Standort.
...
- Nun soll noch berechnet werden, wie viele unterschiedliche Wildtiere auf dem ganzen Areal gezaehlt wurden. Recycelt dazu obenstenenden Code.

### Aufgabe 5: Visualisieren

- __Zeigt eure Ergebnisse in geeigneter Form. Untenstehnd einige Ideen, von denen gerne abgewichen werden kann!__
