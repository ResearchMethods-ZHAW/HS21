---
title: "Code Modeldiagnostics und Darstellung Resultate Uebung"
author: "B.Sigrist"
...
packages <- c("lme4", "bbmle", "MuMIn", "tidyverse", "DHARMa", "car", "MASS", "ROCR", "sjPlot", "rms", "ggeffects", "sjstats", "cowplot", "magrittr", "ggdraw")

ipak(packages)
```
## Hier schnell schnell der Modellfit von letzter Woche als Ausgangspunkt für die heutige Übung
...
```{r tidy=TRUE, eval=FALSE,results= 'hide', fig.keep="none", warning=FALSE, message=FALSE}
...
# use VIF values: if values less then 5 is ok (sometimes > 10), if mean of VIF values not substantially greater than 1 (say 5), no need to worry.
...
p <- plot_model(m_day,type = "pred", terms = "os_scaled [all]") 

labels <- round(seq(floor(min(DF_mod_day$os)), ceiling(max(DF_mod_day$os)), length.out = 7),2)

# check this out: plyr::round_any(seq(floor(min(DF_mod_day$os)), ceiling(max(DF_mod_day$os)), length.out = 7),100)


p <- p + scale_x_continuous(breaks=c(-1.5,-1,-0.5,0,0.5,1,1.5), labels=c(labels))

p
# Funktion um viele Plots auf einem zusammenbringen: cowplot-package (hat auch sonst gute Funktionen fuer gute layouts von Plots)



title <- cowplot::ggdraw() + draw_label("MPG declines with displacement and horsepower", fontface='bold')
...
dies allenfalls weglassen, Sinn oder Unsinn nicht abschliessend nachvollziehbar. 
...
# **Auftrag auf den 17.11.2020: Kurze Vorstellung der Modellresultate & Diagnostics im Plenum und Diskussion der Ergebnisse (keine PP-Praesentation noetig)**
