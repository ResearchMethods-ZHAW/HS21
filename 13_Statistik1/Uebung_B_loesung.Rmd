...
Der $\chi^2$-Test sagt uns, dass die NOVANIMAL-Stichprobe von der Population signifikant unterscheidet ($\chi^2$(`r chi_sq$parameter`) = `r round(chi_sq$statistic[[1]], digits = 3)`, *p* > .001). Auch der exakte Fisher-Test zeigt einen ähnlichen p-Wert (*p* = `r round(fish$p.value, digits = 3)`). Demnach ist unsere Stichprobe im Geschlecht und in der Hochschuzugehörigkeit nicht für die Population repräsentativ. Es scheint, dass die Studentinnen unter- und die Studenten übervertreten sind.
