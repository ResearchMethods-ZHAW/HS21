...
library(gmodels)
CrossTable(nova$member, nova$gender, fisher=T, expected=T)
...
pop_zhaw_exp <- c(.15,.32,.16,.37) # erwartete Verteilung der Population
  
#berechne den Chi-Quadrat-Test
chi_sq <- chisq.test(pop_canteen$tot, p = pop_zhaw$exp_pct)
chi_sq$expected # schaue dir die erwarteten Häufigkeiten an
chi_sq

#Führe einen exacten Fisher Test durch?
...
Der $\chi^2$-Test sagt uns, dass die NOVANIMAL-Stichprobe von der Population signifikant unterscheidet($\chi^2$(`r chi_sq$parameter`) = `r round(chi_sq$statistic[[1]], digits = 3)`, *p* = `r round(chi_qs$p.value, digits = 4)`).
