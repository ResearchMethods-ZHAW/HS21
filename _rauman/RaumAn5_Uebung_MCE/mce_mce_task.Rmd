...
r <- terra::rast(ext(kt_schwyz), 
          resolution = c(25, 25), 
          crs = "EPSG:21781")
ch_raster1 = rasterize(vect(exclusion_areas), r,
                       field = 0) %>% crop(kt_schwyz)
raster_exclusion_areas <- terra::rasterize(exclusion_areas, r, field = 0, background = 1) %>% crop(kt_schwyz)
plot(ch_raster1, main="Exclusion area", axes=FALSE, legend=FALSE, col=terrain.colors(2))
```
