...
draft: false
resources:
  exclude: [dhm25m.tif,eis25m.tif,wind25m.tif,Bewohnte_Flaeche.gpkg,Nationale_Schutzgebiete.gpkg,Waldgebiete.gpkg,Seeflaechen.gpkg,Strassen.gpkg,Untersuchungsgebiet_Schwyz.gpkg]
description: |
...
```{r figmce, echo=FALSE, fig.cap="Process model in ArcGIS Pro ModelBuilder",purl = FALSE, fig.align="center", out.width = '100%'}
...
check_download <- function(files, folder, source = "", url = "https://github.com/ResearchMethods-ZHAW/datasets/raw/main/", cat = TRUE){
  ...
  urls <- paste0("- [",files,"](",full_url,") ",source)
  if(cat){
    cat(urls,sep = "\n")
  }else{
    return(urls)
...
myurls <-check_download(datasets$datasets,"rauman",cat = FALSE)
myurls <- stringr::str_match(myurls,"\\((.+)\\)")[,2]
```


```{r datatable, results='asis', echo=FALSE, purl = FALSE}
datasets %>%
  mutate(
    datasets = kableExtra::cell_spec(datasets, "html", link = myurls)
  ) %>%
...
```{r solutionEx1, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r figrasterizeexclusioncriteria, echo=FALSE, fig.cap="Exclusion area in the canton of Schwyz", fig.align="center", purl = FALSE, out.width="50%"}
knitr::include_graphics("images/mce_exercise_2_exclusion_area.png")
```

```{r solutionEx2, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r figslope, echo=FALSE, fig.cap="Example output of the calculated slope", fig.align="center", out.width="50%", purl = FALSE}
knitr::include_graphics("images/mce_exercise_3_slope.png")
```

```{r solutionEx3, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r figdistancesettlements, echo=FALSE, fig.cap="Example output for the distances to settlements", purl = FALSE, fig.align="center", out.width="50%"}
knitr::include_graphics("images/mce_exercise_4_dist_settlements.png")
```

```{r solutionEx4, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r examplereclassifysettlements}

settlements_max <- terra::global(settlements_ed, "max")[[1]]
...
```{r figreclassify, echo=FALSE, fig.cap="Reclassify values for all criteria", purl = FALSE, fig.align = "center", fig.show = "hold", out.width="49%"}
knitr::include_graphics(c("images/mce_reclassify_1.png", "images/mce_reclassify_2.png"))
```

```{r figdistsettlementsreclass, echo=FALSE, fig.cap="Example output for the reclassified distances to settlements", fig.align="center", out.width="45%", purl = FALSE}
knitr::include_graphics("images/mce_exercise_4_dist_settlements.png")
```

```{r solutionEx5, echo=FALSE, message=FALSE, warning=FALSE}

wind_max <- global(wind25m, "max")[[1]]
...
settlements_max <- global(settlements_ed, "max")[[1]]
reclass_settlements <- c(0,80,0,
                             80,160,0.1,
                             160,240,0.2,
                             240,320,0.3,
...
protected_areas_max <- global(protected_areas_ed, "max")[[1]]
...
forests_max <- global(forests_ed, "max")[[1]]
...
streets_max <- global(streets_ed, "max")[[1]]
...
```{r solutionEx6, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r figweightedoverlay, echo=FALSE, fig.cap="Weighted overall of all criteria", fig.align="center", out.width="50%", purl = FALSE}
knitr::include_graphics("images/mce_exercise_7_weighted_overlay.png")
```

```{r solutionEx7, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r figfinalresults, echo=FALSE, fig.cap="Potential sites for wind power plants in Kt. Schwyz (red = less suitable, green = more suitable)", fig.align="center", out.width="80%", purl = FALSE}
knitr::include_graphics("images/mce_exercise_8_final_plot.png")
```

```{r  solutionEx8, echo=FALSE, message=FALSE, warning=FALSE}
