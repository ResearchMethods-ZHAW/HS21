...
**Schritt 1:** Mit `st_distance()` können Distanzen zwischen zwei `sf` Datensätze berechnet werden. Wird nur ein Datensatz angegeben, wird eine Kreuzmatrix erstellt wo die Distanzen zwischen allen Features zu allen anderen Features dargestellt werden. Wir nützen diese Funktion zur Berechnung der nächsten Nachbarn.
...
**Schritt 3**: Nun müssen wir die Distanzen nach ihrer Grösse sortieren 

```{r, echo = TRUE}
rotmilan_mindist <- sort(rotmilan_mindist) 
```

**Schritt 4**: Jetzt berechnen wir die kummulierte Häufigkeit von jeder Distanz berechnen. Die kummulierte Häufikgeit vom ersten Wert ist 1 (der Index des ersten Wertes) dividiert durch die Anzahl Werte insgesamt. Mit `seq_along` erhalten wir die Indizes aller Werte, mit `lenth` die Anzahl Werte insgesamt.
...
p + 
  ...
  ggrepel::geom_label_repel(aes(x = 0, y = prob, label = paste0(prob*100,"% der Werte...")),min.segment.length = 0,hjust = 1, fill = "lightblue") +
  ggrepel::geom_label_repel(aes(x = res, y = 0, label = paste0("... sind kleiner als ",round(res,0),"m")),hjust = 0,vjust = 1, fill = "lightblue",min.segment.length = 0,inherit.aes = FALSE) +
