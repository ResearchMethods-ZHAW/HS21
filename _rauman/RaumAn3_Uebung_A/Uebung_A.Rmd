...
# aggregiert. Wir started mit der Aggregationsstufe "bezirk"
zweitwohnung_bezirk<- read_sf("zweitwohnungsinitiative.gpkg", "bezirk")

```


```{r}
 ggplot(zweitwohnung_bezirk) +
...
y <- zweitwohnung_bezirk$ja_in_percent
...
```{r, echo = TRUE}
w <- st_touches(zweitwohnung_bezirk, sparse = FALSE)
...
Mit `w[1,]` bekommt  ihr ein Vektor, wo bei allen Bezirke, die den ersten Bezirk (Affoltern) berühren `TRUE` steht und bei allen anderen `FALSE`. Nun können wir überprüfen, ob die räumliche Operation funktioniert hat. 


```{r, echo = TRUE}

beruehrt_affoltern <- w[1, ]

ggplot(zweitwohnung_bezirk[beruehrt_affoltern, ]) +
  geom_sf(aes(fill = BEZIRKSNAME)) +
  geom_sf(data = zweitwohnung_bezirk[1, ], aes(fill = "Affoltern")) # <- ein kleiner Hack damit 
                                                                    #    Affoltern ebenfalls in
                                                                    #    der Legende erscheint

```

Optional: Um über alle Bezirke zu visualisieren, welche sich berühren, könnt ihr folgende Funktion anwenden:
...
An erster Stelle kommt euer `sf` Objekt (`zweitwohnung_bezirk`) und an zweiter Stelle die eben erstelle Matrix (`w`).

```{r, echo = TRUE}
benachbart <- my_connections(zweitwohnung_bezirk,w)
...
Nun könnt ihr Morans $I$ auf der Ebene der Gemeine oder Kantone und untersuchen, ob und wie sich Morans $I$ verändert. Importiert dazu den Layer `kanton` oder `gemeinde` aus dem Datensatz `zweitwohnungsinitiative.gpkg`.
