...
Mit `w[1,]` bekommt  ihr ein Vektor, wo bei allen Kantone, die den ersten kanton (`r sel`) berühren `TRUE` steht und bei allen anderen `FALSE`. Nun können wir überprüfen, ob die räumliche Operation funktioniert hat. 
...
ggplot(zweitwohnung_kanton[beruehrt_1, ]) +
  ...
  # nachfolgend ein kleiner Hack (siehe fill = "Zürich")
  geom_sf(data = zweitwohnung_kanton[1, ], aes(fill = "Zürich")) +
...
Nun könnt ihr Morans $I$ auf der Ebene der Gemeine oder Kantone und untersuchen, ob und wie sich Morans $I$ verändert. Importiert dazu den Layer `bezrik` oder `gemeinde` aus dem Datensatz `zweitwohnungsinitiative.gpkg`.
