---
title: Einführung und Übungen A bis C
output: 
...
- Für *Rasterdaten* exisitert das Package [`raster`](https://rspatial.org/raster/), welches wiederum durch [`terra`](https://rspatial.org/terra) abgelöst wird. Wir brauchen für diese Übung aber nur `raster`, da `terra` relativ neu ist und noch nicht über alle Funktionen verfügt
...
## Übung A Daten importieren und Visualisieren
...
## Übung C: Spatial Joins

### Aufgabe 5: Spatialjoin mit Punkten
...
Danach wird mit `rasterize` der Polygon-Datensatz in ein Raster konvertiert. Mit `field = ` kann festgelegt werden, aus welcher Spalte die Werte des Output Datensatzes entnommen werden sollten. GIS-Experten werden sich erinnern, dass im Gegensatz zu Flächen in einem Vektordatensatz, welche viele verschiedene Attributen haben können, ein Raster nur noch ein Attribut (hier `SKELETT`).
