# Research Methods Demo Statistik 6
# Ordinationen I
# (c) Jürgen Dengler & Egeler Gian-Andrea
...
ggbiplot(o.pca, labels=rownames(mtcars), choices = c(1,2))  # choice gibt die axen an
...
ca.1 <- vegan::cca(sveg^0.5)
ca.2 <- CA(sveg^0.5) # mit FactoMineR
...
summary(ca.2)
plot(ca.2, display = "species", type = "points")
...
# DCA ---------------------------------------------------------------------
dca.1 <- decorana(sveg, mk=10) # segmentierung auf 10 gelegt
plot(dca.1, display = "sites", type = "point")

dca.2 <- decorana(sveg, mk=100) # segmentierung auf 10 gelegt
...
imds <- isoMDS(mde, k=2)
set.seed(1)
mmds <- metaMDS(mde, k=2) # zwei verschiedene packages um eine nmds zu rechnen?
