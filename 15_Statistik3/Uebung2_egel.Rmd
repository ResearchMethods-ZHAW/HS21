...
Führe mit dem Datensatz novanimal.csv eine logistsiche Regression durch. Kann der Fleischkonsum durch das Geschlecht, die Hochschulzugehörigkeit und das Alter erklärt werden?

Hinweise für die Analysen:
1. Generiere eine neue Variable "Fleisch" (0 = kein Fleisch, 1 = Fleisch)
2. entferne fehlende Werte aus der Variable "Fleisch"
3. Führe ein train_test_split durch
4. definiere das Modell und wende es auf den Übungsdatensatz (train.data) an
5. berechne eine Vorhersage mit dem Test-Datensatz (test.data)
6. eruire Modellfit und Modellgenauigkeit
7. berechne eine Konfusionsmatrix und ziehe dein Fazit daraus
...
print(paste('Genauigkeit des Models ist',round(1-misClasificError, digits = 2))) # nahe bei 50%: Model ist passt zwar zu den Daten, aber das Geschlecht noch das Alter kann der Kauf von einem Fleisch-Menü vorhersagen 
...
Bei näher Betrachtung der Daten erkennt man, dass es Personen sind die wiederholt auftau  