...
Heute berechnen wir Moran's $I$ auf verschiedenen Datensätzen und vergleichen die Resultate. Dafür nutzen wir keine Funktion aus einem Package, sondern erarbeiten uns alles selber, basierend auf der Formel von Moran's $I$: 
...
(Lasst Euch nicht davon beirren, dass wir nun `TRUE` und `FALSE` statt `1` und `0` haben. In R sind `TRUE` und `1` äquivalent, sowie auch `FALSE` und `0`). Um zu visualisieren, *welche* Bezirke einander berühren, könnt ihr folgende Funktion anwenden:
