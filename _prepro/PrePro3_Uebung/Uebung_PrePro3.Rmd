...
Lade jetzt nochmal den Datensatz [wetter.csv](https://github.com/ResearchMethods-ZHAW/datasets/raw/main/prepro/weather.csv) herunter und importiere ihn als CSV mit den Korrekten Spaltentypen (`stn` als `factor`, `time` als `POSIXct`, `tre200h0` als `double`).
...
```{r, fig.cap="baseplot mag keine long tables und macht aus den beiden Jahren eine kontinuierliche Linie"}
...
distill::mypurl("Uebung_PrePro3.Rmd")
```
