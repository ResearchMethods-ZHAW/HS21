...
```{r, echo = TRUE}
wasser$skelett_median <- raster::extract(skelett_raster,wasser_2km,fun = median)[,1]

```

```{r}
wasser %>%
  mutate(skelett_median = factor(skelett_median)) %>%
  ggplot() +  
  geom_boxplot(aes(skelett_median,wasserverfuegbarkeit))
```
