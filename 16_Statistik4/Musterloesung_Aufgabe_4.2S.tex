...
\CommentTok{#  sieht die Verteilung zwischen Fleisch und  kein Fleisch }
...
\CommentTok{# erzeugt eine Tabelle mit den beobachteten Fleischesser/Nichtleischesser und den Vorhersagen des Modells}
...
\KeywordTok{dimnames}\NormalTok{(km) <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"kein Fleisch"}\NormalTok{, }\StringTok{"Fleisch"}\NormalTok{), }\KeywordTok{c}\NormalTok{(}\StringTok{"Modell kein Fleisch"}\NormalTok{, }\StringTok{"Modell Fleisch"}\NormalTok{))}
\NormalTok{km}
\end{Highlighting}
\end{Shaded}
