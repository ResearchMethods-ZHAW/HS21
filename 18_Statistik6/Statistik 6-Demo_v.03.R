...
#mit vegan
pca.1 <- rda(raw, scale=FALSE) #Die Funktion rda führt ein PCA aus an wenn nicht Umwelt und Artdaten definiert werden
summary(pca.1,axes=0)
biplot(pca.1, scaling=2)
...
# Mit vegan
pca.2<-rda(sveg^0.25, scale=TRUE)
#Erklärte Varianz der Achsen
summary(pca.2,axes=0)
#PCA-Plot der Lage der Beobachtungen im Ordinationsraum
biplot(pca.2, scaling=1,display = c("sites"),type = c("points"))
#Subjektive Auswahl von Arten zur Darstellung
sel.sp <- c(3,11,23,39,46,72,77,96)
snames <- names(sveg[,sel.sp])
snames
#PCA-Plot der Korrelationen der Variablen (hier Arten) mit den Achsen (h)
scores<-scores(pca.2,display=c("species"))
x<-scores[,1]
y<-scores[,2]
plot(x,y,type="n",asp=1)
arrows(0,0,x[sel.sp],y[sel.sp],length=0.08)
